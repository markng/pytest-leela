...
def describe_MutatingLoader_exec_module():
    """Tests for the bytecode fast-path guards in exec_module."""

    def it_falls_back_to_ast_when_cached_code_is_none():
        """Kills line 239: ``is not → is`` on cached_code guard.

        When cached_code is None, the bytecode path must NOT be taken.
        The AST path should still produce correct results.
        With the mutation (is not → is), cached_code=None would pass the
        guard and try to call _bc_is_supported on None, or skip the
        bytecode path when it should be used.
        """
        import types
        ...
        source = "x = 1 + 2\n"
        # No cached_code — must use AST path
        loader = MutatingLoader(source, mutant, "test_no_cache.py", cached_code=None)

        module = types.ModuleType("test_no_cache")
        loader.exec_module(module)
        # The AST mutation changes + to -, so x = 1 - 2 = -1
        assert module.x == -1

    def it_falls_back_to_ast_when_bc_functions_are_none():
        """Kills line 240: ``and → or`` and ``is not → is`` on _bc_is_supported guard.

        When _bc_is_supported is None (bytecode module failed to import),
        the bytecode path must NOT be taken — AST fallback must work.
        With ``and → or``, the condition would short-circuit past the
        None check and crash on _bc_is_supported().
        """
        import types
        ...
        source = "x = 1 + 2\n"
        # Provide a cached_code so the first guard passes
        cached_code = compile(source, "test_no_bc.py", "exec")
        loader = MutatingLoader(
            source, mutant, "test_no_bc.py", cached_code=cached_code
        )

        # Simulate bytecode module not available
        with (
            patch("pytest_leela.import_hook._bc_is_supported", None),
            patch("pytest_leela.import_hook._bc_is_eligible", None),
            patch("pytest_leela.import_hook._bc_mutate_code", None),
        ):
            module = types.ModuleType("test_no_bc")
            loader.exec_module(module)

        # AST fallback should have applied the mutation: + → -
        assert module.x == -1

    def it_falls_back_to_ast_when_bc_mutate_returns_none():
        """Kills line 245: ``is not → is`` on mutated result guard.

        When _bc_mutate_code returns None (mutation not applicable via
        bytecode), the AST fallback must be used.
        With the mutation (is not → is), a None result would be treated
        as success and exec'd, causing a crash or wrong behavior.
        """
        import types
        ...
        source = "x = 1 + 2\n"
        cached_code = compile(source, "test_bc_none.py", "exec")
        loader = MutatingLoader(
            source, mutant, "test_bc_none.py", cached_code=cached_code
        )

        mock_supported = MagicMock(return_value=True)
        mock_eligible = MagicMock(return_value=True)
        mock_mutate = MagicMock(return_value=None)  # bytecode mutation failed

        with (
            patch("pytest_leela.import_hook._bc_is_supported", mock_supported),
            patch("pytest_leela.import_hook._bc_is_eligible", mock_eligible),
            patch("pytest_leela.import_hook._bc_mutate_code", mock_mutate),
        ):
            module = types.ModuleType("test_bc_none")
            loader.exec_module(module)

        # AST fallback should have applied the mutation: + → -
        assert module.x == -1

    def it_uses_bytecode_path_when_all_guards_pass():
        """Verify the bytecode path IS taken when all conditions are met.

        This is the positive case — when cached_code is not None,
        _bc_is_supported returns True, _bc_is_eligible returns True,
        and _bc_mutate_code returns a code object, the bytecode result
        should be used (not AST fallback).
        """
        import types
        from unittest.mock import MagicMock, patch
        ...
        source = "x = 1 + 2\n"
        cached_code = compile(source, "test_bc_ok.py", "exec")

        # Create a "mutated" code object that sets x = 99
        mutated_code = compile("x = 99\n", "test_bc_ok.py", "exec")

        mock_supported = MagicMock(return_value=True)
        mock_eligible = MagicMock(return_value=True)
        mock_mutate = MagicMock(return_value=mutated_code)

        loader = MutatingLoader(
            source, mutant, "test_bc_ok.py", cached_code=cached_code
        )
        ...
        with (
            ...
            module = types.ModuleType("test_bc_ok")
            loader.exec_module(module)

        # Bytecode path was used — x should be 99, not -1 (AST) or 3 (original)
        assert module.x == 99
